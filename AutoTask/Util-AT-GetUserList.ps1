...
$userQuery = New-ATWSQuery "Resource" "Active" "Equals" "True" "or" "Active" "NotEqual" "True"
$Result = $atws.query($userQuery)
$ATUsers = $Result.EntityResults | Select-Object id,Email, @{Name="Fullname";Expression={$_.FirstName + " " +$_.LastName}}, Active

$ATUsers 
<# 
foreach($userCurr in $ATUsers) {
    # query for open tickets
    Write-Host "querying for id " $userCurr.id
    $ticketQuery = New-ATWSQuery "Ticket" "AssignedResourceID" "Equals" $userCurr.id "and" "Status" "NotEqual" "5"
    $ticketQuery 
    $ticketResult = $atws.query($ticketQuery)
    $ticketResult.EntityResults
