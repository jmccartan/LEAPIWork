# strip html out of description for tickets from Ni2 email processor 
# execute when inbound email received? 
...
# open file 
$ticketsToClose = Import-Csv .\striphtml.csv | Select-Object "TicketNumber"
Write-Host "Beginning to process " $ticketsToClose.Count
Write-Host "========================================="
$ProcessedCount = 0 


Write-Host "Querying tickets  ..."

foreach ($ticketNumber in $ticketsToClose) {
    $ticketQuery = New-ATWSQuery "Ticket" "TicketNumber" "Equals" $ticketNumber."TicketNumber"
    $ticketResult = $atws.query($ticketQuery)

    foreach ($ticket in $ticketResult.EntityResults) {    
...
Write-Host "Complete - Updated Description for " $ProcessedCount " tickets."
